...
`agent-worker-node-smoke.sh` 用于验证 Python Agent Worker 的初始与检索节点执行。它会：

- 启动本地后端 HTTP stub 和真实 FastAPI worker。
- 使用 callback token 调用 `POST /runs/{run_id}/start`。
- 验证 Worker 后台执行 `load_task_context`、`ensure_index`、确定性 `plan_task` 和 `retrieve_context`。
- 验证 Worker 拉取 context、files、symbols、search、file，自动回写 tool call audit，并回写四个 SUCCESS step。
