...
| PRD-F-013 | 自动修复 | RepairAgent 可根据失败日志尝试修复，MVP 限制最多 2 次；当前已支持测试依赖缺失时自动补充 `spring-boot-starter-test` 并重跑沙箱测试 | P1 |
| PRD-F-014 | 人工审批 | diff 必须经用户 Approve 后才能创建 PR | P0 |
| PRD-F-015 | PR 创建 | 系统调用 GitHub API 创建分支、提交和 Pull Request | P0 |
| PRD-F-016 | 实时日志 | 用户能查看 Agent 步骤、工具调用、耗时、状态和错误信息；当前已提供 task/run/step 快照加运行中持续推送的 SSE，前端任务详情会订阅 SSE 并保留轮询兜底，并把已持久化 step JSON 解析为 Agent evidence 面板，展示计划、检索、patch、沙箱测试、自动审查和人工审批 checkpoint 的关键证据；`GET /api/agent/tasks/{id}/run-report` 可生成当前 run 的结构化证据报告和 Markdown，前端支持复制或下载该报告，并可保存运行报告快照，历史快照可复制或下载，便于 rerun 后保留旧报告 | P0 |
