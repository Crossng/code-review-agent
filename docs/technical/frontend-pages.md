...
- 左侧导航使用“发起任务、执行现场、仓库项目、代码洞察、运行数据、系统配置”中文命名；移动端改为顶部横向滚动导航，滚动只发生在导航自身，不允许撑宽页面。
- 当前执行链路固定展示任务、补丁、沙箱测试和 Pull Request 四个受控阶段；主文案翻译为中文，下面保留后端枚举原文，兼顾操作判断和排障。
- 首屏优先展示执行状态和快速发起，运行指标及配置下沉到支撑区，避免核心工作流被概览卡和配置项淹没。
- 视觉采用石墨黑、纸白和安全绿的工程控制台体系；不使用装饰渐变，卡片仅用于表单、重复记录和有明确边界的工具面板。

### 1.2 可用性与响应式约束

- 文档语言为 `zh-CN`，页面只有一个语义化 `main`，提供“跳到主要内容”链接和清晰的标题层级。
- 键盘操作必须显示 `:focus-visible`，异步消息使用 `aria-live`，当前导航使用 `aria-current`；表单控件必须有 label、稳定的 `name`、正确的 `type` 和必要的 `autocomplete`。
- 删除单份接口文档快照和批量清空快照前必须确认，取消确认不得发送删除请求。
- `prefers-reduced-motion` 下关闭平滑滚动；不得使用 `transition: all`、隐藏焦点轮廓或依赖颜色作为唯一状态提示。
- `1080px` 以下主布局改为单列，`760px` 以下采用移动端导航、单列状态链路和双列紧凑指标；长 Java 全限定名、路径、枚举和 JSON 必须在容器内换行或局部滚动。
- `./scripts/browser-smoke.sh` 需要在 1440×1100 和 390×844 视口验收，移动端 `documentElement.scrollWidth` 不得大于 `clientWidth`；失败时输出造成溢出的元素与容器宽度诊断。
...
任务创建区使用原生 radio 语义承载四类任务，中文标签旁保留 `FEATURE`、`BUGFIX`、`REVIEW`、`DOC` 工程枚举，并随选择展示对应的规划重点。桌面端保持一行四项，390px 移动端折为 2×2；任务列表、详情、筛选项和创建成功提示使用同一套中文映射，URL 中仍保存稳定枚举值。
...
| `McpToolSettingsPanel` | 用中文标签展示 MCP 工具目录协议、工具数量、读/写工具、审批门、就绪检查、关键工具和服务地址；工具详情支持按工具名/标题/说明/后端桥搜索，按分类和读写模式筛选，展示筛选命中数量、重置入口、空结果状态、参数 schema、安全规则和 backend bridge；筛选状态通过 `mcpToolQuery`、`mcpToolCategory`、`mcpToolMode` URL 参数恢复，并可复制带 `#settings` 的当前工具视图链接 |
...
- 任务运行中禁止重复启动同一任务。
- 创建任务时必须显式提交当前选择的 `taskType`，不能由前端固定为 `FEATURE`；四类任务都应可通过键盘切换，并显示功能边界、缺陷根因、审查风险或文档事实导向的中文说明。
