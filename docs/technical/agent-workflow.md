...
| `generate_patch` | 生成 unified diff | Python Worker 当前可生成 `WORKER_SAFE_PLANNING_DRAFT` 安全规划草稿 diff，写入 model call audit 和 `patch_record`；主执行链路仍由 Spring Coder recipe catalog / Coder model client / `SAFE_PLANNING_FALLBACK` 兜底，后续再把 Worker patch 接入沙箱安全链路 |
...
Python Agent Worker 当前提供最小服务契约：`GET /health` 返回 worker 健康状态，`POST /runs/{runId}/start` 接收 run 启动请求并返回 MVP graph node 清单。`./scripts/agent-worker-smoke.sh` 会启动或复用 worker，验证该契约并把证据写入 `output/agent-worker-smoke/last-run.json`。Spring Boot 后端已提供可配置的 Worker 启动桥：设置 `REPOPILOT_AGENT_WORKER_ENABLED=true` 后，后台执行会先调用 Worker start API，并记录 `agent_worker_start` step；Worker 调用失败只会写入失败 step，当前主执行链路仍由 Spring Boot 后台 executor 兜底。Worker 侧已提供 `BackendApiClient.record_step(...)`、`record_tool_call(...)`、`record_model_call(...)`、`record_patch(...)` 和 `update_status(...)`，后端对应 `/api/internal/agent-worker/runs/{runId}/steps`、`/tool-calls`、`/model-calls`、`/patches` 与 `/status`，通过 `REPOPILOT_AGENT_WORKER_CALLBACK_TOKEN` 保护，并写入既有 step、tool call、model call、patch record 和状态审计表。Worker 侧还提供 `load_run_context(...)`、`list_project_files(...)`、`read_project_file(...)`、`search_code(...)` 和 `list_symbols(...)`，后端按 `runId` 反查 task/project，作为正式 MCP Server 拆出前的内部工具读取桥；这些 run-scoped 工具读取会自动通过 `/tool-calls` 回写 SUCCESS/FAILED 审计摘要。配置 callback token 后，`/runs/{runId}/start` 会通过轻量图执行器依次执行 `load_task_context`、`ensure_index`、确定性 `plan_task`、`retrieve_context` 和 `generate_patch`，读取 context/files/symbols/search/file，生成 `.repopilot/task-{taskId}-worker-plan.md` 安全规划草稿 diff，通过 `record_model_call(...)` 和 `record_patch(...)` 回写审计与 patch draft，并通过 `record_step(...)` 回写 SUCCESS step；未配置 token 时 `/start` 仍只返回契约响应。后续再把该轻量图执行器替换为真实 LangGraph graph，并把 Worker 生成的 patch 接入安全预检、沙箱测试、风险审查和人工审批链路。
