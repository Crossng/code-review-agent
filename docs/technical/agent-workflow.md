...
Python Agent Worker 当前提供最小服务契约：`GET /health` 返回 worker 健康状态，`POST /runs/{runId}/start` 接收 run 启动请求并返回 MVP graph node 清单。`./scripts/agent-worker-smoke.sh` 会启动或复用 worker，验证该契约并把证据写入 `output/agent-worker-smoke/last-run.json`。Spring Boot 后端已提供可配置的 Worker 启动桥：设置 `REPOPILOT_AGENT_WORKER_ENABLED=true` 后，后台执行会先调用 Worker start API，并记录 `agent_worker_start` step；Worker 调用失败只会写入失败 step，当前主执行链路仍由 Spring Boot 后台 executor 兜底。Worker 侧已提供 `BackendApiClient.record_step(...)` 和 `BackendApiClient.update_status(...)`，后端对应 `/api/internal/agent-worker/runs/{runId}/steps` 与 `/api/internal/agent-worker/runs/{runId}/status`，通过 `REPOPILOT_AGENT_WORKER_CALLBACK_TOKEN` 保护并发布 `STEP_RECORDED`、`TASK_UPDATED` 和可选 `STREAM_COMPLETE` 事件。Worker 侧还提供 `load_run_context(...)`、`list_project_files(...)`、`read_project_file(...)`、`search_code(...)` 和 `list_symbols(...)`，后端按 `runId` 反查 task/project，作为正式 MCP Server 拆出前的内部工具读取桥。配置 callback token 后，`/runs/{runId}/start` 会后台执行 `load_task_context` 和确定性 `plan_task`，读取 context/files/symbols/search 并通过 `record_step(...)` 回写 SUCCESS step；未配置 token 时 `/start` 仍只返回契约响应。后续再把该确定性 runner 提升为真实 LangGraph graph，并继续补齐 `retrieve_context`、tool call 和 model call 回写。
