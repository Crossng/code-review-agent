...
Python Agent Worker 当前提供最小服务契约：`GET /health` 返回 worker 健康状态，`POST /runs/{runId}/start` 接收 run 启动请求并返回 MVP graph node 清单。`./scripts/agent-worker-smoke.sh` 会启动或复用 worker，验证该契约并把证据写入 `output/agent-worker-smoke/last-run.json`。Spring Boot 后端已提供可配置的 Worker 启动桥：设置 `REPOPILOT_AGENT_WORKER_ENABLED=true` 后，后台执行会先调用 Worker start API，并记录 `agent_worker_start` step；Worker 调用失败只会写入失败 step，当前主执行链路仍由 Spring Boot 后台 executor 兜底。Worker 侧已提供 `BackendApiClient.record_step(...)` 和 `BackendApiClient.update_status(...)`，后端对应 `/api/internal/agent-worker/runs/{runId}/steps` 与 `/api/internal/agent-worker/runs/{runId}/status`，通过 `REPOPILOT_AGENT_WORKER_CALLBACK_TOKEN` 保护并发布 `STEP_RECORDED`、`TASK_UPDATED` 和可选 `STREAM_COMPLETE` 事件。后续再把节点执行逐步迁移到 LangGraph worker，并继续补齐 tool call 和 model call 回写。
