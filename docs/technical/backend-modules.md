...
- 可通过 `REPOPILOT_AGENT_WORKER_ENABLED=true` 打开 Agent Worker 启动桥；后台执行会调用 `${REPOPILOT_AGENT_WORKER_URL}/runs/{runId}/start` 并写入 `agent_worker_start` step。若同时配置 `REPOPILOT_AGENT_WORKER_CALLBACK_TOKEN` 且 Worker 启动成功，后端记录 `execution_mode=WORKER_PRIMARY` 并停止本地 executor，等待 Worker callback 回写后续步骤；若缺少 callback token，则记录 `SHADOW_FALLBACK` 并继续 Spring Boot executor；调用失败会记录失败证据并继续本地兜底。
