...
- 配置 `REPOPILOT_AGENT_WORKER_CALLBACK_TOKEN` 后，Python Worker 的 `/runs/{runId}/start` 会后台执行 `load_task_context` 和确定性 `plan_task`，通过内部工具桥读取上下文并通过 `/steps` 回写 SUCCESS 证据；未配置 token 时 `/start` 只返回启动契约。
