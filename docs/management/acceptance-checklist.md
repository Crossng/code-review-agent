...
4. 运行 `./scripts/agent-worker-node-smoke.sh` 验证 Python Agent Worker 初始与检索节点执行、自动 tool call audit 和 step 回写契约。
