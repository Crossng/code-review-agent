...
| AC-011h8 | Agent Worker step 回写可验证 | 后端提供 `POST /api/internal/agent-worker/runs/{runId}/steps` 内部接口，使用 `X-RepoPilot-Worker-Token` 和 `REPOPILOT_AGENT_WORKER_CALLBACK_TOKEN` 鉴权，不需要用户 JWT；请求体使用 `step_name`、`status`、`input`、`output` 和 `error_message`，成功后写入 `agent_step` 并发布 `STEP_RECORDED`；错误 token 返回 `AGENT_WORKER_CALLBACK_FORBIDDEN` 且不落库；Python Agent Worker 提供 `BackendApiClient.record_step(...)`；`AgentWorkerCallbackControllerIntegrationTest` 覆盖真实 MVC/Spring Security/落库路径，`./scripts/agent-worker-callback-smoke.sh` 覆盖 Python client 路径、header、JSON 和响应解析，并将证据写入 `output/agent-worker-callback-smoke/last-run.json` |
