...
| AC-011h16 | Agent Worker 补丁草稿节点可验证 | Python Agent Worker 的轻量图执行器会在 `retrieve_context` 后执行确定性 `generate_patch` 节点，消费 `loaded_context`、`index_status`、`plan_output` 和 `retrieval_output`，生成 `.repopilot/task-{taskId}-worker-plan.md` 的 unified diff，`generationMode=WORKER_SAFE_PLANNING_DRAFT`、`generationProvider=AGENT_WORKER`、`generationModel=worker-retrieval-plan-v1`；节点会通过 `record_model_call(...)` 回写 `generate_patch` model call audit，通过 `record_patch(...)` 持久化 patch draft，并通过 `record_step(...)` 回写包含 patchId、patchStatus、diffPath、diffLineCount 和 evidence 的 SUCCESS step；`./scripts/agent-worker-node-smoke.sh` 覆盖真实 FastAPI Worker + 后端 stub 的五个 SUCCESS step、13 条 SUCCESS tool call audit、1 条 SUCCESS model call audit 和 1 个 `WORKER_SAFE_PLANNING_DRAFT` patch draft |
...
4. 运行 `./scripts/agent-worker-node-smoke.sh` 验证 Python Agent Worker 图执行器骨架、`ensure_index`、初始与检索节点执行、自动 tool call audit、`generate_patch` model call audit、`WORKER_SAFE_PLANNING_DRAFT` patch draft 和 step 回写契约。
