...
## 2026-07-20, Slice 139 - 真实演示体检手册版

真实 token 演示从“靠终端输出判断”升级为“可体检、可留档、可照着走”的中文操作者流程：`real-token-demo-check.sh` 现在会生成脱敏 JSON 证据和 Markdown 操作手册，并把远端 GitHub PR readiness 从简单 token 检查补齐为确认开关、演示仓库 URL、发布开关和 token 四件套。

### Changed

- `scripts/real-token-demo-check.sh` 新增 PASS/WARN/MISS 记录器，默认把体检结果写入 `output/real-token-demo-check/last-run.json` 和 `output/real-token-demo-check/last-run.md`。
- 体检 JSON 记录 `summary`、`readiness`、检查明细、推荐演示命令和安全策略；Markdown 生成中文演示路径、正式演示前环境变量、体检明细和安全说明。
- 远端 GitHub PR readiness 新增 `REPOPILOT_REAL_GITHUB_PR_CONFIRM=create-pr` 和 `REPOPILOT_REAL_GITHUB_PR_REPO_URL` 校验，strict ready 必须同时满足确认开关、github.com demo repo、`REPOPILOT_GITHUB_ENABLED=true` 和 GitHub token。
- 新增 `docs/management/real-token-demo-runbook.md`，覆盖本地闭环、真实 Coder、真实 GitHub PR、Worker Coder 业务闭环、证据文件、安全约束和常见失败处理。
- 文档中心、管理 README、脚本 README、Agent workflow、Sandbox/GitHub 设计、验收清单和 Agent Worker README 同步真实 token 演示手册与体检证据口径。

### Verified

- `bash -n $(rg --files scripts -g '*.sh')` passes.
- `rg --files scripts -g '*.mjs' | xargs -n1 node --check` passes.
- `./scripts/real-token-demo-check.sh` exits `0`; default artifact summary shows `PASS=27`, `WARN=12`, `MISS=0`, `strictPassed=true`, `realCoderReady=false`, `remoteGithubPrReady=false`, and Markdown exists.
- `env -u REPOPILOT_CODER_API_KEY -u OPENAI_API_KEY -u REPOPILOT_GITHUB_TOKEN -u GITHUB_TOKEN REPOPILOT_CODER_MODE=disabled REPOPILOT_GITHUB_ENABLED=false ./scripts/real-token-demo-check.sh --strict` exits `1`; strict missing artifact summary shows `PASS=27`, `WARN=8`, `MISS=4`, `strictFailures=4`, `strictPassed=false`, and Markdown still exists.
- Dummy ready strict check with fake model/GitHub token, `REPOPILOT_REAL_GITHUB_PR_CONFIRM=create-pr` and `REPOPILOT_REAL_GITHUB_PR_REPO_URL=https://github.com/Crossng/demo-repopilot.git` exits `0`; artifact summary shows `PASS=35`, `WARN=4`, `MISS=0`, `realCoderReady=true`, `remoteGithubPrReady=true`, and the fake model/GitHub token strings are not present in JSON.
...
- 在真实 GitHub token 和可丢弃 demo 仓库环境中执行 `./scripts/real-github-pr-demo.sh`，留存真实远端 PR URL、分支、commit 和演示截图。

## 2026-07-20, Slice 138 - Worker双业务闭环Smoke版

Worker 业务闭环 smoke 从单一 `GET /api/users/summary` 扩展为两个独立真实业务场景：第一个场景继续验证 Coder 模型 429 后恢复、结构化 `retryAudit` 和 run report 的 `Worker 重试恢复证据`；第二个场景新增 `GET /api/users/names` 名称列表接口，验证 Worker Coder 会按不同任务生成不同 Java 业务 diff，并且无 retry 时不会误报恢复证据。

### Changed

- `scripts/agent-worker-business-smoke.mjs` 改为 `businessScenarios` 驱动，顺序运行 `summary` 和 `names` 两个独立 demo Spring 项目/任务/run。
- 模型 stub 会根据 Coder prompt 识别业务场景，默认请求状态为 `[429, 200, 200]`：第一次 `summary` 触发 429 注入，恢复后返回汇总接口 diff，随后 `names` 返回名称列表接口 diff。
- 每个场景仍断言当前任务只有 1 个 Worker primary patch，且都必须通过 `validate_patch_safety`、`apply_patch`、`run_tests`、`review_patch`、`waiting_human_approval`、用户审批和本地 `DRAFT_READY` PR 草稿链路。
- `output/agent-worker-business-smoke/last-run.json` 新增 `scenarioCount=2`、`scenarios` 和 `modelRequest.scenarioHints`，同时保留首个场景的顶层字段用于兼容旧证据读取习惯。
- 脚本 README、Agent Worker README、Agent workflow 和验收清单同步双业务闭环 smoke 口径。

### Verified

- `node --check scripts/agent-worker-business-smoke.mjs` passes.
- `bash -n scripts/agent-worker-business-smoke.sh` passes.
- `./scripts/agent-worker-business-smoke.sh` passes for `agent-worker-business-smoke-1784519641-49742@example.test`.
- Summary scenario passed with task `#3763`, patch `#1557`, draft PR branch `repopilot/task-3763`, commit `4f60e4672fa7365aaeaaf911b10ce41ad405edf1`, `retryAudit.attemptCount=1` and `retryReportSection.key=worker_retry`.
- Names scenario passed with task `#3764`, patch `#1558`, draft PR branch `repopilot/task-3764`, commit `d57aa355ab9ab0f10c9f97bbf16607b057f1d5f9`, `retryAttemptCount=0` and no retry report section.
- Business smoke evidence shows `scenarioCount=2`, model request statuses `[429, 200, 200]`, and `scenarioHints=["summary","summary","names"]`.
...
- 继续把 Worker Coder 从 stub 双场景推进到真实 token 演示质量：保留 parser、安全预检、Docker 沙箱、风险审查、人工审批、run report 和本地/远端 PR 证据链。

## 2026-07-20, Slice 137 - Worker业务重试报告Smoke版

Worker 业务闭环 smoke 继续补强 retry 证据链：真实 Worker primary 业务 run 中的 Coder 模型 429 恢复，不只要出现在结构化模型审计 API，也必须进入后端 run report 的 `Worker 重试恢复证据` section 和 Markdown。

### Changed

- `scripts/agent-worker-business-smoke.mjs` 新增 `assertRunReportRetryEvidence(...)`，默认注入 Coder 429 时要求 run report 包含 `worker_retry` / `Worker 重试恢复证据` section。
- Business smoke 现在断言 run report facts 包含 `重试失败尝试：1`、`恢复调用：1/1` 和 `模型调用：1`，highlights 与 Markdown 同时包含 `模型 generate_patch` 和 `HTTP 429`。
- `output/agent-worker-business-smoke/last-run.json` 新增 `retryReportSection`，保存该报告小节的 key、title、summary、facts 和 highlights，便于回放证据。
- 脚本 README、Agent Worker README、Agent workflow 和验收清单同步业务闭环 smoke 的 run report retry section 验收口径。

### Verified

- `node --check scripts/agent-worker-business-smoke.mjs` passes.
- `bash -n scripts/agent-worker-business-smoke.sh` passes.
- `./scripts/agent-worker-business-smoke.sh` passes for `agent-worker-business-smoke-1784518641-8968@example.test`, task `#3707`, patch `#1536` and draft PR branch `repopilot/task-3707`.
- Business smoke evidence `output/agent-worker-business-smoke/last-run.json` shows `retryReportSection.key=worker_retry`, title `Worker 重试恢复证据`, facts `重试失败尝试：1` / `恢复调用：1/1` / `模型调用：1`, and highlight `模型 generate_patch ... HTTP 429`.
- Smoke cleanup leaves `0` `agent-worker-business-smoke-%` users and `0` `Worker Coder 业务演示%` tasks.
- `PYTHONPATH=. python3 -m unittest discover -s tests` passes in `agent-worker`.
- `mvn -q -Dmaven.repo.local=../.m2 test` passes in `backend`.
- `npm run build` passes in `frontend`.
...
- 继续扩展更多 Worker Coder 真实业务 diff 场景，并保留 parser、安全预检、Docker 沙箱、风险审查、人工审批和 run report 诊断证据。
